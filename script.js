...
const instructors = [
  ...
  for (let i = 0; i < instructors.length; i++){
    if (instructors[i].availability === "all" || instructors[i].availability === "weekend"){
        if (instructors[i].specialities.includes("Javascript")){
            myInstructors.push(instructors[i]);
        }
    }
  }

for (let i = 0; i < myInstructors.length; i++){
    if (myInstructors[i].specialities.includes("Python")){
        console.log("Hi " + myInstructors[i].name + " , we inform you that this weekend you will be doing the support class and you need to prepare a Python workshop");
        
    }else{
        console.log("Hi " + myInstructors[i].name + " , we inform you that this weekend you will be doing the support class");
    }
}
